...
def _get_most_desired_combination(samples_with_combination):
    ...
    for combination, evidence in samples_with_combination.items():
        ...
        """
        !!!!! HERE !!!!!
        if we set order=1, the number_of_combinations would be always 1, so the 2nd conditon (after or) would be false forever
        TO FIX THIS, we add a new condition as (best_number_of_combinations==number_of_combinations and best_support_size > support_size),
        SO if order=1, the label combination with samllest support_size would be divided firstly
        """    
        if (currently_chosen is None) or (best_number_of_combinations < number_of_combinations and best_support_size > support_size) or (best_number_of_combinations == number_of_combinations and best_support_size > support_size):
...
class IterativeStratification(_BaseKFold):
    ...
    def __init__(self, n_splits=3, order=1, sample_distribution_per_fold = None, shuffle=False, random_state=None):
        ...
        self.order = order
        super(IterativeStratification, self).__init__(n_splits, shuffle=need_shuffle, random_state=self._rng_state if need_shuffle else None)
